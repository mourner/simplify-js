...
t('simplifies points correctly with the given tolerance', (t) => {
    const result = simplify(points, 5);
    t.same(result, simplified);
    t.end();
});

t('just return the points if it has only one point', (t) => {
    const result = simplify([{x: 1, y: 2}]);
    t.same(result, [{x: 1, y: 2}]);
    t.end();
});

t('just return the points if it has no points', (t) => {
